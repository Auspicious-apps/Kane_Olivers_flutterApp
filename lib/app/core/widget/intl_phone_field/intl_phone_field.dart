...
class IntlPhoneField extends StatefulWidget {
  ...
  /// ```
  final Rx<Country> initialSelectedCountry;
  ...
  void initState() {
    ...
    if (widget.autovalidateMode == AutovalidateMode.always) {
      final initialPhoneNumber = PhoneNumber(
        countryISOCode: widget.initialSelectedCountry?.value.code ?? '',
        countryCode: '+${widget.initialSelectedCountry?.value.dialCode}',
        number: widget.initialValue ?? '',
      );
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (ctx, setState) => CountryPickerDialog(
          ...
          selectedCountry: widget.initialSelectedCountry.value,
          onCountryChanged: (Country country) {
            widget.initialSelectedCountry.value = country;
    ...
    return Obx(() {
      if (number.startsWith('+')) {
        number = number.replaceFirst(
            RegExp(
                "^\\+${widget.initialSelectedCountry?.value.fullCountryCode}"),
            "");
      } else {
        number = number.replaceFirst(
            RegExp("^${widget.initialSelectedCountry?.value.fullCountryCode}"),
            "");
      }
        ...
        children: [
          Container(
              decoration: BoxDecoration(
                 border:Border.all(color:Color.fromRGBO(24,34, 38, 1),),
                borderRadius: BorderRadius.circular(50),color: Color.fromRGBO(24,34, 38, 1)
                  ),
              child: _buildFlagsButton()
                  .paddingSymmetric(horizontal: 3, vertical: 10)),
          SizedBox(width: 7),
          Expanded(
            child: TextFormField(
              ...
              onSaved: (value) {
                widget.onSaved?.call(
                  PhoneNumber(
                    countryISOCode:
                        widget.initialSelectedCountry.value.code ?? '',
                    countryCode:
                        '+${widget.initialSelectedCountry.value.dialCode}${widget.initialSelectedCountry.value.regionCode}',
                    number: value!,
                  ),
                );
              },
              ...
              validator: (value) {
                if (value == null || value.isEmpty || !isNumeric(value))
                  return widget.emptyFieldMessage;
                if (!widget.disableLengthCheck) {
                  return value.length >=
                              widget.initialSelectedCountry.value.minLength &&
                          value.length <=
                              widget.initialSelectedCountry.value.maxLength
                      ? null
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                if (widget.showCountryFlag) ...[
                  ...
                  //     ?
                  AssetImageWidget(
                    'lib/app/core/widget/intl_phone_field/assets/flags/${widget.initialSelectedCountry.value.code.toLowerCase()}.png',
                    imageWidth: 25,
                    imageHeight: 25,
                  ...
                  if (widget.enabled &&
                    ...
                    widget.dropdownIcon ??
                        Icon(Icons.arrow_drop_down, color: AppColors.appColor),
                ...
                if (widget.showDivider) ...[
                  Container(
                    width: width_0point5,
                    height: height_25,
                    color: AppColors.appBorderDarkColor,
